...
const EditProfileModal: React.FC<EditProfileModalProps> = ({ visible, onClose, initialProfileData }) => {
  const { user, updateUser } = useContext(AuthContext);
  const [name, setName] = useState(initialProfileData?.name || "");
  const [description, setDescription] = useState(initialProfileData?.description || "");
  const [link, setLink] = useState(initialProfileData?.link || "");
  const [showInstagramBadge, setShowInstagramBadge] = useState(initialProfileData?.showInstagramBadge || false);
  const [isPrivate, setIsPrivate] = useState(initialProfileData?.isPrivate || false);
    ...
    return (
      <View style={styles.modalContainer}>
        <View style={styles.header}>
          ...
          <TouchableOpacity onPress={handleConfirmEdit} style={styles.headerButton}>
      ...
      <View style={styles.contentContainer}>
        <View style={[styles.section, styles.nameSection]}>
          <View style={styles.sectionText}>
            ...
            <View style={styles.nameRow}>
              <Ionicons name="lock-closed-outline" size={16} color="#888" style={styles.lockIcon} />
        ...
        <TouchableOpacity onPress={handleEditBio} style={styles.section}>
          <View style={styles.sectionTextFull}>
            ...
            <Text
              style={[styles.tappableText, !description && styles.placeholderText]}
              numberOfLines={3}
              ellipsizeMode="tail"
            >
        ...
        <TouchableOpacity onPress={handleEditLink} style={styles.section}>
          <View style={styles.sectionTextFull}>
            ...
            <Text
              style={[styles.tappableText, styles.linkText, !link && styles.placeholderText]}
              numberOfLines={1}
              ellipsizeMode="tail"
            >
              {link || "Add link..."}
        ...
        <View style={styles.switchSection}>
          <View style={styles.switchTextContainer}>
            ...
            <Text style={styles.description}>
              When turned on, the Threads badge on your Instagram profile will also appear.
            </Text>
          </View>
          <Switch
