...
import { Image, Share, StyleSheet, Text, TouchableOpacity, useColorScheme, View } from "react-native";
...
export default function Post({ item }: { item: Post }) {
  ...
  return (
    <TouchableOpacity style={styles.postContainer} onPress={() => handlePostPress(item)} activeOpacity={0.8}>
          ...
          <View style={styles.usernameContainer}>
            <TouchableOpacity onPress={() => handleUserPress(item)}>
              <View style={styles.usernameRow}>
                ...
                <Text style={[styles.timeAgo, colorScheme === "dark" ? styles.timeAgoDark : styles.timeAgoLight]}>
                  {item.timeAgo}
                </Text>
              </View>
            </TouchableOpacity>
      ...
      <View style={styles.postContent}>
        <Text style={[styles.postText, colorScheme === "dark" ? styles.postTextDark : styles.postTextLight]}>
          {item.content}
        </Text>
        {item.image && <Image source={{ uri: item.image }} style={styles.postImage} resizeMode="cover" />}
        {item.location && item.location.length > 0 && <Text style={styles.postText}>{item.location.join(", ")}</Text>}
      </View>
      ...
      <View style={styles.postActions}>
        ...
        <TouchableOpacity style={styles.actionButton}>
          ...
          {item.reposts > 0 && <Text style={styles.actionCount}>{item.reposts}</Text>}
        </TouchableOpacity>
        <TouchableOpacity style={styles.actionButton} onPress={() => handleShare(item.username, item.id)}>
